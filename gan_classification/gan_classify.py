...
from gan_classification.gan_m2 import VAE,M2
from gan_classification.gan_utils import init_weights,training_M2,test_M2,training_VAE
import torch
from collections import defaultdict
from gan_classification.gan import gen
import time
...
if __name__ == '__main__':
    start=time.time()
    gen.load_state_dict(torch.load('../gan_classification/gen.pth'))
    print(m2)
    writer = SummaryWriter('class_gan')

    while epoch<EPOCH:
        epoch+=1
        print("\nepoch: ",epoch)
        training_M2(m2,gen,labelled,optimizer,cuda,training_data,training_data_labelled,training_data_unlabelled,writer,epoch)
